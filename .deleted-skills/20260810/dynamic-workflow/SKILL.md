...
completion_criteria:
  ...
  - "Worker-Flotte nach Aufgabe und Verfügbarkeit gewählt (sol-pruefer=Urteil, kimi=Gegenperspektive, luna-worker=Mechanik, sonnet-worker/haiku-worker ergänzend) — keine Pflicht-Kombination, NIE Fable-Subagents"
...
3. **Script bauen** nach der geteilten Vorlage
   ...
   parsen (`typeof args === 'string' ? JSON.parse(args) : args`). Worker per
   `agentType` **frei nach Aufgabe und Verfügbarkeit** wählen (seit 25.07.2026;
   die frühere Pflicht-Kombination Sol+Kimi+Luna ist gestrichen, weil sie bei
   Anbieter-Ausfällen die Arbeit blockierte): `sol-pruefer` für Urteil,
   `kimi-recherche`/`kimi-worker` für eine unabhängige Perspektive,
   `luna-worker` für Mechanik/Tests, `sonnet-worker`/`haiku-worker` ergänzend.
   Vor dem Start prüfen, welche Routen antworten — keine tote Flotte anfordern.
   Verifier möglichst aus anderer Familie als der Schreiber; geht das nicht,
   im Bericht vermerken. NIE Fable-Subagents, kein `Date.now()`/`Math.random()`.
