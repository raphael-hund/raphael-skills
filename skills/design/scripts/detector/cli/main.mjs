...
async function detectCli() {
  ...
  // AENDERUNG GEGENUEBER DEM ORIGINAL, 30.07./01.08.2026: Ziele, die nicht
  // gescannt werden konnten, blieben still und die CLI meldete Exit 0 — also
  // "keine Funde" fuer einen Lauf, der nie stattfand.
  const scanFehler = [];
  ...
  if (allFindings.length > 0) {
    ...
    if (primary.length > 0) process.exit(2);
  }
