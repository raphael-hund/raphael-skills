...
Concrete signals for each tell. `scripts/scan-ai-slop.mjs` encodes these; this
file is the human reference and the place to widen patterns for a specific stack.
...
non-English slop. `scripts/rules.ru.mjs.example` is a shipped Russian example
(kept as a template, not wired in — see the German-rules TODO in SKILL.md).
Remember
