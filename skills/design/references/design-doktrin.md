...
## 8. Vier-Kategorien-Trennung & Visual-QA-Disziplin `[omo]`

Idee/Methodik uebernommen aus oh-my-openagent (github.com/code-yeongyu/
oh-my-openagent, Sustainable Use License 1.0 — nur interner Agentur-Gebrauch,
kein Weiterverkauf/SaaS). Nur die Trennungslogik + QA-Disziplin, kein Code
(die referenzierten 70 Marken-Systeme/12-Taste-Skills liegen dort in leeren
Git-Submodulen und wurden NICHT nachgezogen).

**Vier saubere, orthogonale Kategorien statt einer Regel-Wand:**
1. **Ausfuehrungs-Philosophie** (minimal/brutalist/soft/Redesign) — hoechstens
   EINE pro Projekt, entspricht unserer taste-/ui-ux-Register-Wahl.
2. **Marken-Token-Quelle** (Referenz-Brand/-Site als Vorbild) — frei
   stapelbar mit (1); entspricht `design-dna-schema.md`.
3. **Lookup-Tool** (Paletten/Fonts/Patterns nachschlagen) — reine
   Werkzeug-Ebene, keine Haltung; entspricht `ui-ux-db-nutzung.md`.
4. **Performance-Gate** — echter Browser (Playwright-Chromium), NIE die
   `lighthouse`-CLI direkt; Fix auf Architektur-Ebene (z.B. Bild-Preload,
   Bundle-Split), NIEMALS durch Entfernen von Animation/Content erkauft.

**Visual-QA Dual-Oracle-Disziplin** (ergaenzt die impeccable-Detektoren, die
nur Code sehen, nicht das gerenderte Ergebnis):
- **Coverage-Pflicht:** jede Seite/jeder State pruefen, nie eine Stichprobe —
  ein Fehler auf einer nicht geprueften Seite zaehlt als Gesamt-Fail.
- **Frische-Pflicht:** ein Screenshot/Capture aelter als die letzte
  Quell-Aenderung ist ungueltig, nie als PASS werten.
- **Capture-Hygiene vor Reviewer-Dispatch:** Dateisignatur/Kompositions-Fehler
  selbst pruefen, bevor ein (Sub-)Reviewer Zeit daran verschwendet.
- Ein automatisiertes Skript, das 0 Findings meldet, ersetzt den
  Blick-Check nicht — es zielt den Reviewer, ersetzt ihn nicht.
