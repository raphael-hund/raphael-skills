...
1. Read `references/vault-config.md` for the installed vault path. On this VPS it is `/root/raphael-brain/`; read its `AGENTS.md` before capture or compile. Raw and candidate writes use the broker, canonical writes the authorized promoter. Local write gates override the generic examples below.
