...
## Raphael-Brain integration

For Raphael’s AI OS, first read `/root/raphael-brain/ais-os/README.md` and its applicable `AGENTS.md`. These local mappings override starter-kit defaults: use the existing vault, `ais-os/intake.md`, `ais-os/priorities.md`, `ais-os/connections.md`, and `ais-os/interviews/`; never create parallel context or bypass the Brain write/promotion policy. Use the existing policy-integrated `apps/3d-brain` rather than scaffolding it again. If the user explicitly selects another project, follow that project’s routes instead.
...
**Output of Phase 2:** a dated decision record with the five answers, autonomy level and KPI. In Raphael-Brain, use the existing `scripts/decision-register.py` candidate route with an accepted provenance-bearing source; never append to the script or create a second decision log. In a fresh standalone starter project, use `decisions/log.md`.
...
1. **One dated decision record** with the Method spec, using the project-specific register above
...
8. **Limit edits to the project-specific decision record and the selected artifact.** An explicitly selected audit repair may update its existing workflow or routing files; preserve unrelated content. Other files remain read-only.
