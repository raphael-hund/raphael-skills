...
function main() {
  ...
  if (flags.help) {
    console.log('Aufruf: node run-evidence.mjs create --out DIR --run-id ID --repo PATH --revision SHA --base-url URL');
    console.log('                            [--plan DIR] [--plan-required] [--contract role=path]... [--phase PHASE]');
    console.log('         node run-evidence.mjs bind-build --out DIR --revision SHA');
    console.log('         node run-evidence.mjs attach --out DIR --kind plan|g1|sweep|ship --path FILE');
    console.log('         node run-evidence.mjs validate --out DIR [--ready]');
    console.log('Exit 0 = RUN_EVIDENCE=PASS, 1 = BLOCKED, 2 = Aufruf.');
    process.exit(0);
  }
