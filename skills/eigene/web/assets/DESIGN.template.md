...
For selected library components: [source/version and local files, native HTML/build-time render/client island, hydration directive and reason, primitive/provider/portal boundaries, token mapping, usable no-JS/load-failure fallback, actual JS/CSS requests and raw/gzip size, state evidence]. Preserve existing customized components; use one implementation per component job. A bounded island does not imply a whole-site React app.
