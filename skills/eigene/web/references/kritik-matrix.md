...
2  SITE     je eine Leaf über ALLE Fold/Key-Shots (Überlappung zu PAGE):
            ...
            images   → multi-family kimi-recherche  | claude-only Sonnet-Instanz
...
3  LENS     je eine Leaf über alle Folds, andere Frage als PAGE/SITE:
            ...
            copy        → multi-family kimi-critic    | claude-only Sonnet-Instanz
            seo         → multi-family kimi-recherche | claude-only Sonnet-Instanz
                          (onpage-check.mjs + Shot: Titel, H1)
            trust       → multi-family kimi-recherche | claude-only Sonnet-Instanz
...
- **multi-family:** vier Familien sind Pflicht, sobald mehr als eine Route
  existiert — Grok, Kimi, Opus, GPT (Sol nur Code-Ursache zu überlebenden
  visuellen Befunden, keine Bildpfade an Sol). Masse >40 Shot-Pfade: Luna.
