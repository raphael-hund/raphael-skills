...
Drei Chats. `web` in allen. Kritik und Bau starten im ersten Turn einen Dynamic Workflow über das Workflow-Tool, Profil multi-family.
...
- **STATUS.md** hält je Arbeitspaket eine Zeile: `[offen|in-arbeit|gebaut|
  ...
  `gelesen-von` nennt das Leaf (z. B. `grok-critic`), nie „Parent hat reingeschaut“.
...
|---|---|---|---|
| **Plan** | nur `/web` | high | zusätzlicher Workflow, Slash `/ultracode`, `/website-plan`, `/design`, `/visual-aaa`. website-plan: 0 Children. |
| **Kritik** | `/web` + Workflow | high | Code; PNG-Dump; Slash `/ultracode`. Flotte = `kritik-matrix.md`. |
| **Bau** | `/web` + Workflow | Ultracode-Session-Default | Slash `/ultracode`; `/design`; PNG-Dump; neue Kritik erfinden |
...
```
/web — Kritik, client-<name>. Controller, kein Builder.
...
```
/web — Bau, client-<name>. Du bist Controller, nicht der Builder.
...
2. grok-critic (Grok) und kimi-critic unabhängig auf denselben Fold/Hover-Shots
3. grok-worker nur für belegte Code-Ursachen (datei:zeile), Input = Shot-Pfad + CSS-Ausschnitt
4. astra-worker, dann kimi-worker oder fable-builder für jede Copy — Opus schreibt nie Text
