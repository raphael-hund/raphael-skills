...
Render → Self-Read → `visual-g1.py` Exit 0 → Agent `grok-critic` (andere Familie)
...
`grok-critic` / Grok / Kimi; der Parent/Controller öffnet oder liest nie Bilder.
...
Besetzung kommt aus `agent-roster.md`. **Nie Haiku.** Builder der Seite ist `opus-builder` — dann ist A = Grok und B **nicht** Opus (gleiche Familie). B = `kimi-critic`. Sol prüft Code, nicht die Shots. Nie A=B, nie dieselbe Familie wie der Builder.

| Rolle | Fähigkeit | Liest | Auftrag |
|---|---|---|---|
| Code-Kritik („Zoll") | `sol-critic` | Befundliste + Source als Textausschnitt (keine Bildpfade) | „Verifiziere pro Befund die Code-Ursache (datei:zeile) oder widerlege ihn. Keine Design-Meinung ohne Code-Beleg." |
| Visuelle Kritik A | Grok / `grok-critic` | manifest.json + Shots **dieser einen Seite** | „Kritisiere Hierarchie, Spacing, Typo, Bildschnitt, CTA-Fuehrung pro Shot. Befund + Shot-Datei als Beleg. `biggest_gap` einer Vorschau ist visuell — nie Bewertungszahl, Custom-Domain/Vercel oder SLA-Stunden (das ist FAKT-GATE / fakt-park)." |
| Visuelle Kritik B | `kimi-critic` (andere Familie als A und als Builder) | dieselben Shots wie A | „Zweite, unabhaengige Sicht. Befund + Shot-Datei als Beleg. Dieselbe biggest_gap-Regel: Fakten-Nits parken, Bild zuerst." |
...
überlebenden Befunde aus der ersten Stufe-2-Kritik und, nur nach FAIL, dem nächsten Kritikdurchlauf auf die Fixliste.
