# Fold-Duell: Raphael sieht drei React-Folds vor dem Routenbau
...
6. **Ein Stufe-1-Leaf je Richtung, 25 Minuten:** `fable-builder`, `astra-worker` und `kimi-worker` bauen drei disjunkte React-Routen `/duell/<slug>` im selben Projekt aus `assets/react-starter`. `opus-builder` ergänzt nur bei Ausfall oder als zusätzliche vierte technische Variante nach ausdrücklicher Entscheidung; nie statt aller drei primären Familien. Keine losen HTML-Dateien. Gates: Typecheck, Produktionsbuild, `detect.mjs`, Sweep 1440×900 + 390×844.
...
```
Exit 0 nur mit Abschnitt `## Fold-Duell` in `PLAN.md` (drei Zeilen `slug | Achse | Referenzbilder | Copy-Quelle`). Das Fold-Duell ist eine eigene Phase: `session-gate --rolle fold-duell`, drei React-Routen bauen, Montage liefern, dann harter Stopp. `--rolle bau` bleibt gesperrt, bis `DECISIONS.md` eine GO-Zeile mit «Fold-Duell» trägt. Die Bau-Phase beginnt erst nach der Wahl und baut die gewählte Richtung als Design-System und Routen aus.
...
// Primäre Familien: Fable, Astra, Kimi. Jeder Aufruf bindet agentType als Literal.
const folds = await parallel([live, glow, beweis]) // fable-builder, astra-worker, kimi-worker
...
async function glow() { return agent(`${LEAF}\n${COMMON}\nFOLD-DUELL Richtung «${RICHTUNGEN[1].slug}».`, { label: 'duell:glow-karten', phase: 'Fold-Duell', agentType: 'astra-worker', schema: RESULT }) }
async function beweis() { return agent(`${LEAF}\n${COMMON}\nFOLD-DUELL Richtung «${RICHTUNGEN[2].slug}».`, { label: 'duell:beweis-editorial', phase: 'Fold-Duell', agentType: 'kimi-worker', schema: RESULT }) }
