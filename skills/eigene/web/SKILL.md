...
Kritik- und Bau-Session **müssen** `/web` plus `/orchestrate` fahren.
`/ultracode` zusätzlich ist redundant — Orchestrate ist der Owner.

**Drei Sessions** (verbindlich, Raphael 01.09.2026): **Plan** / **Kritik** /
**Bau**. Nicht zwei Modi in einem Chat. Handoff-Dateien und Chips:
`references/planner-executor-protokoll.md`. Spawn-Plan der Kritik:
`references/kritik-matrix.md`. Plan schreibt `PRUEFGEGEN.md` (Skills,
Referenzen, Linsen) — ohne die Datei startet Kritik nicht. Kritik schreibt
KRITIK-n.md, Bau setzt nur Überlebende um. Parent nie CSS, nie PNG-Dump.
Kritik-PNG-Read = Leaves. Shot-Ledger in STATUS.md. Ohne Re-Sweep nach Fix
= nicht geprüft.
...
6. **Vorschau vor Launch (hart, 01.09.2026):** Raphael zeigt dem Kunden eine geile
   Vorschau. Preview-Blocker sind langsam: **Ablauf, Sitemap, Idee, Design**.
   Inhalt ist ein Swap — Satz, Wort, Bild, Sektion, Review-Platzhalter, 50 vs 60,
   24 vs 28h, Domain/Vercel. Das parkt, das ist kein `biggest_gap`. Launch bleibt
   hart, wenn erfundener Proof als echt rausgeht. Klassifizierer:
   `node scripts/preview-befund-klasse.mjs "<befund>"` — Vorschau-`biggest_gap`
   nur bei `preview: block` (visual-block oder struktur-block).
...
Inhalt (Satz, Wort, Bild, Sektion, Review-Platzhalter) und unklare Zahlen sind
`FAKT-GATE` / content-park — kein `OWNER-BLOCKER`, keine Vorschau-Sperre.
Launch darf erfundenen Proof nicht als echt ausliefern.
...
3. **copy** — sektionsweise. Vorschau: Working-Copy, Platzhalter-Reviews und
   offene Zahlen erlaubt (`content-park`). G2-Judge und echte Proofs erst Ship.
...
- **Inhalt statt Design.** Satz, Wort, Bild, Sektion, ausgedachte Reviews,
  50 vs 60, Domain/Vercel — Swap, `content-park` / `ops-park`. Vorschau-Blocker
  sind Ablauf, Sitemap, Idee, Design. Klassifizierer:
  `scripts/preview-befund-klasse.mjs`.
