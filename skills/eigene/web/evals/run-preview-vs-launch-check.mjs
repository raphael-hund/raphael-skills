...
const PLAN = process.env.WEBSITE_PLAN_SKILL || path.join(WEB, "references", "rolle-plan.md");
const VALIDATOR = process.env.WEBSITE_PLAN_VALIDATOR || "/nonexistent/validate-plan.py";
const KONTRAKT = process.env.VISUAL_AAA_KONTRAKT || "/root/raphael-skills/skills/eigene/visual-aaa/references/kritiker-kontrakt.md";
...
zeile(
  /müssen/.test(skill) && /Dynamic Workflow/.test(skill) && !/Liste nicht nachladen/.test(skill),
  "Slash-Dump wirft den Workflow-Hinweis nicht mehr weg",
);
zeile(
...
if (fs.existsSync(PLAN) && fs.existsSync(VALIDATOR)) {
  ...
  zeile(true, "website-plan-Skill entfaellt seit web 4.0.0; Plan-Phase liegt in references/rolle-plan.md");
}

if (fs.existsSync(KONTRAKT)) {
  const kontrakt = fs.readFileSync(KONTRAKT, "utf8");
  zeile(
    /biggest_gap[\s\S]{0,240}Fakten/.test(kontrakt),
