...
2. **Modell / agentType** — Luna, Sol, Terra, Haiku, Sonnet, Kimi K3, …
...
| `haiku-worker` | Claude | Massen-Read, Parse, Klassifikation, billige Lint-Schleifen | standard |
...
|---|---|---|
| Mechanik + Tests | `luna-worker` + `haiku-worker` oder `sonnet-worker` | Luna tippt, Claude spot-checkt |
| Feature-Bau | `sonnet-worker` oder `kimi-worker` + `luna-worker` (Tests) + Verifier andere Familie | Bauen ≠ Prüfen |
| Frontend / DE-Copy | `kimi-worker` + `luna-worker` oder `haiku-worker` + Review Claude/Sol | Kimi-Stärke + Cross-Review |
...
| Nur Lesen / Map | `haiku-worker` + optional `kimi-recherche` | billig + Gegenblick |
...
- Nur `model:'haiku'`-Overrides ohne GPT/Kimi-agentType → zählt nicht.
