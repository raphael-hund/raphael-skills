...
## React default (since 08.09.2026)

A retrieved 21st component is installed directly into the Next.js project through the `@21st` namespace in `components.json` (`params.api_key` from `API_KEY_21ST`; the `headers` variant returned 401) or the install command the search result prints. Verified 08.09.2026: `npx shadcn@latest add @21st/tommyjepsen/pricing-section-with-comparison` installed the item plus its `badge`/`button` registry dependencies, and `next build` with `output: "export"` rendered it into static HTML. Inspect imports (Framer Motion, Lucide, Next-specific modules) and add `"use client"` where the item relies on state or effects. Record source, licence, adaptation and removed dependencies in DESIGN.md. Legacy HTML sites keep [component-islands.md](component-islands.md).

### CLI and Design Context (verified 08.09.2026)

`npx @21st-dev/cli@latest` 1.17.0: `whoami` confirmed the saved login, `usage` reported tier paid with unlimited search and retrieval, `search "faq accordion" --type c --limit 3 --json` returned live items with install commands. `init --design-context` created `.21st/design.json` and `.21st/DESIGN.md` from `app/globals.css` and `components/ui`; `search --context auto` and the MCP `get_inspiration` then rerank against those tokens. Refresh after token changes. The 21st CLI skills for publishing (`21st-registry`, `21st-design-sync`) are outward-facing and run only on explicit instruction. Full connection table: [component-registries.md](component-registries.md).
