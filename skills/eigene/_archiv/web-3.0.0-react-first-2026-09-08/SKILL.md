---
name: web
description: Build or redesign credible React service-business websites (Next.js App Router, TypeScript, Tailwind v4, shadcn/ui with namespaced registries), with implemented SEO subpages, reference-informed DESIGN.md and independent browser review. Install real components from the connected registries (21st.dev MCP and CLI, shadcn MCP, Mantine MCP, Magic UI, Aceternity, Kibo and the further catalogued libraries) instead of rebuilding them. Analyse supplied websites into source-linked patterns. Integrate GPT Image website imagery and separate assets with content/style references, IMAGE-SPEC and optional Scrollcraft motion; search and place logos, icons and reference photos with the bundled image-search script; choose components from a 50-library catalogue with licence, registry namespace and install route; implement interface motion natively from the Emil Kowalski rules; pick gallery references from the 56-source catalogue and Refero MCP. Build every UI surface from the measured design-depth chapters via their router. Also create or reconcile DESIGN.md. Use for Raphael's website builds, reference-learning requests and substantial frontend SEO work; preserve narrower scopes.
metadata:
  raphael-version: "3.0.0"
  raphael-archived: "Archiv vom 08.09.2026 ohne corpus/studies/design-depth; Nachfolger skills/eigene/web 4.0.0"
  raphael-changelog: '["2.0.1 (07.09.2026): Rückverweis-Gate vor Versionssprüngen, Reveal-Regel für content-visibility, Headless-QA bei View Transitions, Referenzrollen-Regel für Bildserien", "2.0.2 (07.09.2026): §8a Review-Identität (Backend-Revision binden), §8b Domain→Projekt-Bindung vor Deploy, Dokumentarfoto-Inhaltsfreigabe in §6", "2.0.3 (07.09.2026): §8c Tracking+Datenschutz als eine Lieferung, §8d Fortschritt am sichtbaren Produkt berichten", "2.1.0 (07.09.2026): Design-Depth-Module Anatomie, Effekte, UX-Entscheidungen, Quellenindex und datierte Studien", "2.2.0 (07.09.2026): 14 tiefe Design-Depth-Kapitel mit Bauanleitungen und Stilfamilien, Router references/design-depth.md, 54 Einzelanalysen unter studies/design-depth/deep, Kernregeln in den Kurzmodulen", "3.0.0 (08.09.2026): React-first (Next.js App Router, Tailwind v4, shadcn-Registries mit Namespaces), geprüfter Starter assets/react-starter, component-registries.md mit verifizierten Registry-/npm-/MCP-Anbindungen, 21st-CLI und Design Context, shadcn- und Mantine-MCP registriert; HTML-first und Astro-Inseln nur noch als Legacy für bestehende Projekte"]'
  raphael-design-depth: "2.2.0: 14 deep chapters (spacing/layout, color, typography, buttons, cards, tables/data, mobile, effects, motion, imagery, forms, navigation, states/proof/pricing, commonalities) with build recipes and style families, routed via references/design-depth.md; 54 measured single analyses; anatomy, effects and UX modules carry the top-10 rules per chapter"
  raphael-class: "F"
  raphael-scope: "agency"
  raphael-sensitivity: "internal"
  raphael-loads: '["references/stack.md", "references/component-registries.md", "references/21st-dev.md", "references/image-to-code.md", "references/image-search.md", "references/ui-libraries.md", "references/motion-native.md", "references/inspiration-galleries.md", "references/dependencies.md", "references/design-depth.md", "references/design-anatomy.md", "references/design-effects.md", "references/design-ux-decisions.md", "references/design-depth.md"]'
  raphael-requires-skills: '["design@^0","copywriting@^0","seo@^0","higgsfield@^0"]'
  raphael-completion-criteria: '["Alle vereinbarten Seiten und SEO-Unterseiten existieren als prerenderte React-Routen, deren Response-HTML echte Inhalte, Metadaten und interne Links enthält", "Wiederverwendbare Komponenten stammen aus den konfigurierten Registries (components.json, 21st, Mantine) mit Herkunft in DESIGN.md; Eigenbau ist als Entscheidung festgehalten", "DESIGN.md ist mit dem tatsächlichen CSS, den Assets und dem gerenderten Verhalten abgeglichen", "Desktop- und Mobilaufnahmen, Roh-HTML, Statuscodes, Links, Formulare und Bewegung wurden am gebauten Stand geprüft und Abweichungen behoben oder benannt", "Logos, Icons und Bilder sind mit Quelle im Manifest eingebaut; fehlende Zugriffe oder ungeprüfte Eigenschaften sind ausdrücklich genannt", "Veröffentlichung nur im beauftragten Umfang, danach Prüfung der ausgelieferten URLs"]'
...
For new builds and substantive component work, confirm the component connections before the first component: `claude mcp list` shows `21st`, `shadcn` and `mantine` connected; `API_KEY_21ST` is readable in this session (`test -r /root/.secrets/api-keys.env`, otherwise run `heal-secrets-acl.sh`); the project has `components.json` with its namespaces and `.21st/design.json`. Then actually use the 21st MCP and CLI according to [21st-dev.md](references/21st-dev.md) and the registries in [component-registries.md](references/component-registries.md). Record a concrete connection/authentication gap if unavailable and continue authorised work. A skill entry or MCP URL is not an active connection; do not manufacture successful usage or repeatedly request setup. Before editing the skill source itself, run a write probe on the directory; an ACL mask of `r-x` blocks writes despite a named `rwx` entry (Observation 0061).
...
Read [stack.md](references/stack.md). Default output since 08.09.2026: **a React project on Next.js App Router with TypeScript, Tailwind v4 and shadcn/ui**, started from the verified [react-starter](assets/react-starter/README.md). Every SEO route is a prerendered page whose response HTML carries content, metadata and links; use `output: "export"` unless the project needs a server. Pages stay Server Components; interaction lives in client components. HTML-first and Astro islands remain only as the [legacy route](references/component-islands.md) for targeted work on existing HTML sites, never for new builds. Preserve a viable existing React stack for targeted work. For ChatGPT Sites, follow actual Sites building/hosting instructions and framework/runtime constraints.

Reusable UI is **installed, not rebuilt**. Read [component-registries.md](references/component-registries.md): it holds the verified registry namespaces for `components.json`, the npm packages, the GitHub sources and the MCP servers (21st, shadcn, Mantine) and the search order per component job. Search the configured registries with the shadcn CLI or MCP and 21st with its Design Context before writing a component by hand; record the chosen source, licence and adaptation in DESIGN.md, or record the decision to build locally when nothing fits.

For interface motion, apply [motion-native.md](references/motion-native.md): Emil Kowalski's animation rules translated into CSS transitions, `@starting-style`, WAAPI and native View Transitions, with duration/easing tokens, reduced-motion handling and the situation table for navigation, dialogs, cards, scroll reveal, page changes and form feedback. In the React stack, `motion/react` is the normal choice for gesture physics, layout transitions and presence; CSS transitions and WAAPI remain right for simple hover, fade and reveal. Scroll-driven media remain under [Scrollcraft](references/scrollcraft.md).
...
For new or edited images and individual raster assets, apply the integrated [GPT Image module](references/gpt-image.md). Distinguish complete images from independently placed assets; supply actual content and style reference files with explicit roles, record photographic observations separately from desired settings, and use the [image/asset library](references/image-library.md) selectively. Use the native image tool or the user-authorised Higgsfield route, verify files and alpha, then compose real React elements with semantic markup. For image-to-video and scroll-controlled media, follow the [Scrollcraft adapter](references/scrollcraft.md) with an inspected still fallback.

Build foundations and shared components, then compose pages in bounded sections. For full builds and redesigns load [design-depth.md](references/design-depth.md) with its anatomy, effects and UX modules before the first component, and take each section's build recipe from the matching chapter in the Design-Depth Router (in 4.0.0: references/design-depth.md). For any reusable component need, first run the search order in [component-registries.md](references/component-registries.md), then consult [ui-libraries.md](references/ui-libraries.md) for licence, warnings and the per-library profile in the corpus JSON (its A–E routes are legacy labels; in the React stack the question is only registry install, npm install or unsuitable). Install actual source through the configured namespaces; adapt to DESIGN.md tokens and retain existing local components. Keep typography, imagery and interactions coherent without repeating one layout mechanically. Prefer authentic brand imagery; generated illustrations cannot masquerade as staff, premises or completed projects.

Carry selected reference patterns into named component jobs. Use the 21st MCP tools the host exposes and the `21st` CLI with the project's `.21st/design.json` (search, get_inspiration, get_component, generate), then inspect provenance, permitted reuse, dependencies and behaviour. Restyle useful components to DESIGN.md; retain accessible semantics and truthful content. Reuse the chosen implementation across page families. Do not add spectacle or call the MCP repeatedly for an unchanged need.

Deliver indexable content and crawlable links in initial HTML: prerendered Server Components, no client-only shells for public content. Verify with `curl` on the built output, not with the dev server. Isolate browser-only interaction in client components. Implement meaningful statuses, metadata, canonicals, sitemap, robots, responsive images and truthful applicable schema. Check installed framework APIs.
