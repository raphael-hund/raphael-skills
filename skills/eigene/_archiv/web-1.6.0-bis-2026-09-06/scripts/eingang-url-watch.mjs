...
letzten N Tage. Dry-run schreibt nichts und startet keinen Capture.`;
...
const result = { dryRun, scannedFiles: candidates.length, urls, created: 0, skipped: 0, failed: 0, ledgerUpdated: false };
...
function ledgerHosts(raw) {
  return new Set((raw.match(/https?:\/\/[^\s|]+/g) || []).map((url) => {
...
if (!dryRun) {
  ...
  const recorded = ledgerHosts(ledger);
  for (const row of urls) {
    const casePath = path.join(outDir, `${row.slug}.md`);
    if (fs.existsSync(casePath) || recorded.has(new URL(row.url).hostname.toLowerCase().replace(/^www\./, ''))) {
      result.skipped++;
      continue;
    }
    ...
    ledger += `| ${row.url} | \`${row.slug}\` | \`${row.source}\` | ${status} |\n`;
    recorded.add(new URL(row.url).hostname.toLowerCase().replace(/^www\./, ''));
    result.created++;
    if (code !== 0) result.failed++;
  }
  if (result.created > 0 || !fs.existsSync(ledgerPath)) {
...
if (jsonMode) console.log(JSON.stringify(result, null, 2));
  ...
  console.log(`${dryRun ? 'Dry-run: ' : ''}${urls.length} URL(s), ${result.created} neu, ${result.skipped} vorhanden, ${result.failed} Capture-Fehler.`);
