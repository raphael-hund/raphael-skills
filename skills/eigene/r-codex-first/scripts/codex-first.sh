...
if run_seat /root/.codex-1 >"$OUTFILE" 2>&1; then
  ...
  if run_seat /root/.codex-2 >>"$OUTFILE" 2>&1; then
    ...
    echo "[codex-first] Nächster Schritt: selbst bauen oder an Kimi (r-kimi-first) geben." >&2
    exit 1
  fi
fi
...
echo
echo "[codex-first] JETZT CLAUDE: Diff streng reviewen (r-code-review), Tests SELBST fahren,"
