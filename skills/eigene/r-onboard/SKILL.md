...
loads: [references/loop1-ablauf.md, references/intake-grill.md]
requires_skills: [r-offers@^0, r-copywriting@^0, r-eval@^0]
completion_criteria:
...
# r-onboard — Loop 1: Kunden-Onboarding
...
3. **icp-synth** — ICP-Synthese (Sonnet, Verifier Sol). G2. **Jede Behauptung mit Beleg.**
   Output: `client-<name>/wiki/ICP.md` (Vorlage icp-template).
4. **offer-architect** — Angebots-Architektur → ruft `r-offers`. Fable, Checkpoint Raphael.
   Output: `OFFER.md`.
5. **proof-miner** — Proof-Mining (Sonnet, Verifier Sol). **Keine unbelegte Zahl (Block).**
   Output: `PROOF.md` — jede Zahl mit raw-Zitat + Quelle.
6. **brand-voice** — Voice-Analyse → ruft `r-copywriting` (voice-analysis.md-Vorlage). Sonnet, G2.
   Output: `VOICE.md`.
