...
Ein Video-Skript für einen Kunden soll klingen wie **dessen** Referenz-Skripte.
Eine aufgesagte Regel-Zusammenfassung verfehlt das. Sprechtext, der wie eine aufgesagte Guideline klingt
...
1. **Markt-Referenzen (im Skill):** vor dem Schreiben **zwei wörtliche Beleg-Skripte
   derselben Bauform** aus `hook-formeln.md` / `skript-architekturen.md`
   (beide in diesem Ordner) lesen. Das sind Zeilen aus 682 echten, laufenden Ads. Zusätzlich Stichprobe aus der
...
[sprech-sprache.md](sprech-sprache.md):
gemessen an 682 echten Ads. Für Orwell-DE-Grundlagen, Floskel-Verbotsliste und
Sprachstil-Auswahl bleibt der `copywriting`-Skill Primärquelle (extern, falls
installiert); die Spoken-Schicht ist
...
- **Regel-Sätze**: Sätze, die eine Marketing-Regel paraphrasieren statt etwas zu sagen,
  ...
  Kommunikation, die Vertrauen aufbaut." Beispiel gut (Referenzkonto-Referenz): "Wir bringen deine
...
## no-ai-slop als Gate (falls installiert)

Nach jedem fertigen Skript-Entwurf läuft — **falls der externe `no-ai-slop`-Skill
installiert ist** — ein Zweit-Editor-Pass über den
...
`sprech-sprache.md`), sind **keine** Slop-Muster: nicht wegpolieren. Ist no-ai-slop
nicht installiert, ersetzt der lokale `text-check.py` (`$ADS_ROOT/scripts/text-check.py`)
den Slop-Check nicht vollständig, bleibt aber Mindest-Gate.

## Claims-Verbote gehören dem ads-Router

HWG/UWG- und Meta-Policy-Verbotsliste liegt in `../claims-verbote.md`.
Dieser Ordner schreibt Skripte, prüft sie aber nicht selbst final gegen
die Verbotsliste: claims-qa läuft laut `ads`-Ablauf in einer **frischen Session, anderer
Modellfamilie** (Sol; externer Skill, falls verfügbar), nie als Selbstprüfung des Autors (Regel 8). Beim Schreiben trotzdem
...
einer vorhandenen Dossier-Datei (Proof/VOC) stammen (Grounding: siehe
`../craft/dossier-schema.md` bzw. `../teil-research.md`). Disclaimer direkt hinter Monster-Zahlen (P5 in
