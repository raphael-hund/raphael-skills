# Beat-Struktur und Aufbau — Skelett, Varianten, ABT

Kondensiert aus `wiki/craft/ads/creative/2026-07-20-referenz-video-skript-struktur.md`
(kanonisch, 277-Ads-Basis; externes Wiki, nicht Teil des Skills) und `wiki/_candidates/konsolidiert-2026-07-23/
video-skript-strukturen-hooks.md` (Stand 23.07., `status: candidate`; extern,
Übergangsquelle).
Hook-Familien selbst NICHT dupliziert — sie stehen in `hook-formeln.md` (dieser Ordner).

## Das Standard-Skelett (5 Beats, 277-Ads-Basis)

Hook → Problem/Mechanismus → Proof → Offer → CTA. Median 5 Beats, Median 159 Wörter
(voll strukturierte Skripte: Median 195 Wörter).

| Beat | Position (Median) | Funktion | Ø-Anteil |
|---|---|---|---|
| Hook | 1 (102/102) | Selbst-Selektion durch Spezifität | früh |
| Problem/Mechanismus | 0,34–0,38 | benanntes System als Unique-Mechanism | mittel |
| Proof | 0,52 (Mitte) | Social Proof + Zahl | Mitte |
...
Aus `raw/evidence/2026-07-23-ads-wissenspaket-2/web-abt-framework/analyse.md` (externer
Wiki-Rohbestand, nicht Teil des Skills). Kein
...
statt nur aneinandergereiht zu sein ("und dann, und dann" = flach, kein ABT). Nützlich als
Diagnose-Werkzeug beim Redigieren: Übergänge markieren, wenn kein Bruch da ist, fehlt Spannung.

## Alternative Skript-Frameworks (Werkzeugkasten, kein Ersatz)

Aus dem Candidate-Text, für Fälle, in denen das 5-Beat-Skelett nicht passt:
...
- **Hook-Body-CTA (Minimalgerüst):** Hook = Avatar direkt ansprechen, Body = Problem+Lösung
  in einem Satz, CTA = konkrete Handlung. Für schnelle Tests/Low-Fi.
- **VSL-Grundstruktur (10 Blöcke, 7-12 Min):** für lange Formate, nicht Standard-Ad-Länge.

## Awareness-Stufen als Hook-Bauplan (Eugene Schwartz)

Unaware → Neugier/Loophole. Problem-aware → Problem verschärfen/Alternative. Solution-aware
→ Ergebnis + Mechanismus. Product-aware → Proof + Unique Mechanism. Most-aware → Angebot/
Incentive für sofortigen Kauf. Bild und Text müssen zur selben Awareness-Stufe passen
(Mismatch zerstört die Ad).
...
Jeder Hook muss von jemandem verstanden werden, der verschlafen um 8 Uhr morgens scrollt,
ohne zweimal nachzudenken: absolut einfache Wörter, keine Fachbegriffe, keine
Schachtelsätze, ein Gedanke pro Satz. Problem darf komplex sein, Sprache nie. Prüf-Fragen
vor Freigabe: (1) versteht das jemand ohne Branchenkontext? (2) jedes Wort ein Alltagswort?
(3) passt der Satz in einen Atemzug? (4) fühlt sich die Zielperson direkt angesprochen?
(5) ist das Problem klar benannt, ohne dass die Formulierung selbst schwer ist?

## Bridge-Übergänge — häufigster Bruchpunkt
