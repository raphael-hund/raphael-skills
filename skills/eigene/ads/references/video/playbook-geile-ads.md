...
Craft-Referenzen dieses Ordners (`hook-formeln.md`, `skript-architekturen.md`,
...
(`/root/02 CLIENTS/<slug>/ads/playbook-<slug>.md` — externe Konvention, Kundenrepo
außerhalb des Skills), nicht hierher.
...
1. **Winner des Kunden laden: falls vorhanden.** Existiert
   `/root/02 CLIENTS/<slug>/ads/korpus/eigene-ads.md` (externe Konvention, Kundenrepo), die Top-Ads nach
...
5. **Kein Weichmacher-Cluster:** individuell, maßgeschneidert, ganzheitlich, auf Augenhöhe,
   innovativ. Beleg: `sprech-text-regeln.md` + no-ai-slop-Verbotsliste (externer Skill,
   falls installiert).

### no-ai-slop — nach Entwurf (falls installiert)
Jeder Sprechtext läuft durch `no-ai-slop` **Edit-Modus**, sofern der externe Skill
installiert ist (sonst genügt der lokale `text-check.py`). Häufige Ads-Slop-Muster:
...
10. no-ai-slop Edit-Pass dokumentiert (Was geändert / keine Muster), falls der Skill installiert ist?
...
- Ein eigener Performance-Korpus ist optional und liegt beim Kunden:
  `/root/02 CLIENTS/<slug>/ads/korpus/eigene-ads.md` (externe Konvention, Kundenrepo). Fehlt er, ist das kein Fehler:
...
- Refresh eines Kunden-Korpus nur auf ausdrücklichen Auftrag mit
  autorisiertem Zugang; der Skill liefert dafür kein Skript mehr mit.
