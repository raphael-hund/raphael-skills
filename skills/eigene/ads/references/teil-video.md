...
## Produktions-Defaults Video & Person (Entscheid Raphael 18.09.2026)

- **Wer vor der Kamera steht** (Kunde oder Mitarbeiter:in) wird **nicht aktiv
  getestet**. Die Hypothese «Frau funktioniert generell meistens besser»
  bleibt eine Hypothese, keine Regel (D-Liste in `konflikt-register.md`).
- **Dialekt oder Hochdeutsch: egal.** Der Kunde bzw. die Sprechperson wählt.
- **Länge: egal.** Ziel-Richtwert: **150–250 Wörter Sprechtext ≈ 45–90
  Sekunden**. Das ersetzt keine inhaltliche Begründung — Aufgabe und
  Belegmaterial führen weiterhin (die 20–40/60–90-Sekunden-Planungshilfen
  unten bleiben als Untergrenzen-Orientierung).
- **Starke Claims sind erlaubt, wenn der Kontext mitkommt** — etwa «Du
  verdienst Geld im Schlaf» nur mit erklärendem Mechanismus daneben, nie als
  nackte Heilsversprechen-Zeile. Die Compliance-Liste in
  `claims-verbote.md` bleibt unverändert verbindlich.
- **Studio oder Handy: egal.** Handy-Selfie bleibt der Default aus der
  Doktrin (Korpus: 41 % Selfie), ist aber **kein Dogma** — Studio ist keine
  begründungspflichtige Ausnahme mehr.
- **Ein unsicher wirkender Kunde im Video ist kein Fail-Kriterium.** Nicht
  coachen, nicht ersetzen, nicht blockieren — die Daten entscheiden.
...
4. **Wie das Video aussieht.** Lade genau zwei Dateien:
   `craft/video.md` (Kanon: IDs, Hook-Lagen, Aufbau) und
   `video/video-visuals.md` (Formatwahl und Aussehen).
   Nicht den ganzen Ordner. Ohne diese zwei Dateien kein Skript.
5. **Passendes Wissen.** Über `INDEX.md` die zur Testfrage passende
...
Diese vier Lagen sind das **Produktions-Modell** des Hooks (wo er physisch
sitzt). Die inhaltliche **Copy-Zerlegung** desselben Hooks — Person /
Situation / Intention plus Committed-vs-Curious-Filter — steht in
`hook-werkstatt.md` §1. Zwei Ebenen, kein Widerspruch: hier wird gebaut,
dort wird der Inhalt geprüft.
...
Wahl und Aussehen stehen in `video/video-visuals.md`.
...
Füllgerüst (video:A2 kurz, der häufigste Video-Default):
...
High-Ticket mit mehreren Cases: video:A1 (Case-Stack).
Geo plus kleiner Check: video:A6.
...
- video/video-visuals.md war geladen
...
## Aus dem Leaf ads-video (Merge 18.09.2026)

Der Leaf-Skill ads-video ist aufgegangen; sein SKILL.md war ein 28-Zeilen-Stub
ohne eigene Doktrin. Übernommen wurden seine einzigen Zusatz-Regeln:

- **Optionales externes Gate:** `copywriting/scripts/forbidden-check.py` auf dem
  Skript, Exit 0 — nur falls der externe copywriting-Skill installiert ist
  (sonst genügt der lokale `text-check.py` aus dem Gate oben).
- **Kein Second Brain nötig** für die Video-Arbeit: alles Nötige liegt in diesem
  Skill.
- **Tiefe liegt im Ordner `video/`:** `hook-formeln.md` (video:F1–F13),
  `skript-architekturen.md` (video:A1–A7 + Messlatte), `beat-struktur-und-aufbau.md`,
  `sprech-sprache.md`, `sprech-text-regeln.md`, `strategien-taktiken.md`,
  `video-visuals.md`, `voice-dna-ads.md`, `playbook-geile-ads.md`,
  `skript-analyse-methodik.md`. Markt-Korpus: `korpus/` (scripts-711.jsonl kanonisch,
  referenz-ads.md als generierter Export).
