...
# r-watch — Video-Analyse lokal (yt-dlp + ffmpeg, null API-Kosten)
...
1. **Extrahieren** — Helper lädt nach `/tmp` und sampelt in zwei Stufen:
   ...
   - **Transkript:** YouTube → `--write-auto-subs` (de/en, als .srt) — kein Whisper
     nötig. IG/TikTok → bewusst **Frames-only**, der On-Screen-Text trägt die Botschaft.
